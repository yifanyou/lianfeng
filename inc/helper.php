...
function wphy_script_style(){
	...
	wp_enqueue_style( 'ui',get_template_directory_uri() . '/css/ui.css', array(), '1.0' );
	...
	wp_enqueue_script( 'owl.carousel', get_stylesheet_directory_uri() . '/js/owl.carousel.js', array('jquery'),'1.0.0');
	wp_enqueue_script( 'jquery.fancybox', get_stylesheet_directory_uri() . '/js/jquery.fancybox.js', array('jquery'),'1.0.0');
...
function the_crumbs() {
	...
	echo get_the_term_list( $post->ID, 'products' ,'','>',' > ');
		...
		elseif ( is_page() && !$post->post_parent ) { // 页面
			...
			echo $before ;
			printf( __( '搜索词为: %s', 'xs' ),  get_search_query() );
			echo  $after;
		} elseif ( is_tag() ) { //标签 存档
			echo $before ;
			printf( __( '标签归档: %s', 'xs' ), single_tag_title( '', false ) );
			echo  $after;
